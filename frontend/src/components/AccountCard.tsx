...
export function AccountCard({ account, onDelete, onEdit, deleting }: AccountCardProps) {
    ...
    <div className="group bg-slate-800 rounded-xl p-4 flex items-center justify-between gap-4 border border-slate-700/50 card-hover">
      ...
      <div className="flex items-center gap-1 shrink-0">
        <button
          ...
          className="text-slate-400 hover:text-white transition-all duration-200 p-2.5 rounded-xl hover:bg-slate-700 btn-press group/btn"
          title="Editar conta"
        >
          <PencilIcon className="w-5 h-5 transition-transform group-hover/btn:scale-110" />
        </button>
        <button
          onClick={() => onDelete(account.id)}
          disabled={deleting}
          className="text-slate-400 hover:text-rose-400 disabled:opacity-40 transition-all duration-200 p-2.5 rounded-xl hover:bg-rose-500/10 hover:shadow-lg hover:shadow-rose-500/10 btn-press group/btn"
          title="Remover conta"
        >
          {deleting ? (
