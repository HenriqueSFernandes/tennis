...
app.use(
  ...
  cors({
    origin: (origin) => {
      ...
      if (origin.includes('localhost') || origin.startsWith('http://127.0.0.1')) return origin;
      return null;
    },
...
app.post('/api/accounts', async (c) => {
  ...
  const summary = await addAccount(db, username, password, displayName, phone, appPassword);
  ...
  const results = await Promise.allSettled(
    accounts.flatMap((acc) =>
      [1, 2].map(async (courtId) => {
        ...
        if (!pwd) return null;
        const current = await getCurrentBooking(db, acc.id, stored.username, pwd, courtId);
